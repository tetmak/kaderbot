...
export function ChatBot({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // DÜZELTME: previousAnalysis KULLANMA, sadece chat history gönder
      const response = await fetch('/api/analyze', {
        ...
        body: JSON.stringify({
          ...
          isPremium: true, // Chatbot için premium
          payload: {
            ...context.userData,
            // Analiz sonucunu özet olarak gönder
            previousAnalysisType: context.analysisType,
            userName: context.userData.firstName
          },
          // ÖNEMLİ: Chat history düzgün formatta gönder
          chatHistory: messages.map(m => ({
            role: m.role,
            content: m.content
          }))
          // previousAnalysis parametresini KALDIR
        })
      });
